...
void navigateToRegister(context){
  ...
  emit(LoginLoadingState());
    ...
    await http.post(Uri.parse('$baseUrl/api/v1/auth/login'),
        body:jsonEncode({
      "subject" : subject,
      "password" : password,
    ...
    if(responseBode['status']=='UNAUTHORIZED'|| responseBode['status']== 'BAD_REQUEST'|| responseBode['status']== 'UNSUPPORTED_MEDIA_TYPE'){
...
void getSearch(String value){
  ...
  search =[];
  DioHelper.getData(url: '/api/v1/cities/search',
  query: {
    'q':'$value',
  },
...
async {
  emit(LoginLoadingState());
  http.Response response =
  await http.post(Uri.parse('$baseUrl/api/v1/auth/logout'),
      body:jsonEncode({
        "subject" : subject,
        "password" : password,
        "device_token": deviceToken
