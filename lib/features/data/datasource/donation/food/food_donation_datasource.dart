...
class FoodDonationDataSource {
  ...
  Future<FoodDonationResponse?> create(
    ...
    final response = await client.post(Uri.parse('$baseUrl/api/v1/donations/food'),
  ...
  Future<List<FoodDonationResponse>?> search(String query) async {
    final response =
    await client.get(Uri.parse("$baseUrl/api/v1/donations/food"));
  ...
  Future<FoodDonationResponse?> upvote(int id) async {
    final response = await client.put(
        Uri.parse('$baseUrl/api/v1/donations/food/$id/upvote'),
        headers: {
          'Content-Type': 'application/json',
  ...
  Future<FoodDonationResponse?> updateImage(int id, Uint8List file) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      return FoodDonationResponse.fromJson(jsonDecode(body));
    } else if (response.statusCode == 400) {
